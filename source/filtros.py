...
def sujetosCandidatos(sujetos,listaSujetos):
    candidatos = []
    for s in sujetos:
        prec1 = round(precisionSujeto(nlp(str(s)),str(listaSujetos[0][1])),3)
        prec2 = round(precisionSujeto(nlp(str(s)),str(listaSujetos[1][1])),3)
        prec3 = round(precisionSujeto(nlp(str(s)),str(listaSujetos[2][1])),3)
        sumaPrecs = round(prec1 + prec2 + prec3,3)
        promedio = sumaPrecs / 3
        if (prec1 < promedio) & (prec2 < promedio) & (prec3 < promedio): sujetos.remove(s)
        else: 
...
def generarRequerimientos(sujetos,roles):
    requerimientos = []
    for s in sujetos:
        for r in roles:
...
def filtrarRequerimientos(listaRequerimientos):
    for r in listaRequerimientos:
        p0 = nlp(str(r[0]))
        p1 = nlp(str(r[1]))
        p2 = nlp(str(r[2]))
        p4 = nlp(str(r[1]) + ' ' + str(r[2]))
        if (p4.similarity(p0) < 0.8) & (p1.similarity(p2) < 0.8) : listaRequerimientos.remove(r)
    return listaRequerimientos

# Dado un texto, lista los posibles sujetos del mismo y los filtra
def listarSujetos(texto): #listarSujetos
    tuplas = []
    for token in texto:
        if (token.is_stop == False) & (token.dep_ != 'punct') & (token.dep_ == 'nsubj'): tuplas.append((token.text,token.pos_,token.dep_))
    palabras = []
    for t in tuplas:
        palabras.append(str(t[0]))
    return palabras
